...
// Resilient in-memory storage fallback for Vercel/demo environments where DATABASE_URL is not set
const globalFallbackSlots: SlotDto[] = [
  {
    id: 'slot_sonnet4_01',
  ...
  },
  {
    ...
    remainingCredit: 1_200_000,
    dailyQuotaValueUsd: 40.00,
    suggestedDailyPriceUsd: 8.40,
  ...
  },
  {
    ...
    activeRental: {
      ...
      status: 'ACTIVE',
      expiresAtUtc: `${getTodayUtcString()}T23:59:59.999Z`,
      usedQuota: 14200,
    },
  },
];

function createInMemorySlot(
  modelType: ModelId,
  providerWallet: string,
  apiKey: string,
  dailyQuota: number,
  remainingCredit: number,
  pricing: ReturnType<typeof calculateIdleKeyPrice>
): SlotDto {
  const catalog = MODEL_CATALOG[modelType];
  const maskedKey = maskApiKey(apiKey);
  const newSlot: SlotDto = {
    id: `slot_${modelType.replace(/[^a-zA-Z0-9]/g, '')}_${Date.now()}`,
    providerWallet,
    modelType,
  ...
  globalFallbackSlots.unshift(newSlot);
  return newSlot;
}
...
export async function GET() {
  ...
  if (hasDbUrl) {
    try {
      ...
      // Merge newly added in-memory slots with DB slots
      const dbIds = new Set(slotDtos.map((s) => s.id));
      const newlyAdded = globalFallbackSlots.filter((s) => !dbIds.has(s.id));
      const allSlots = [...newlyAdded, ...slotDtos];
      ...
      // If DB fails, fallback to resilient in-memory storage below
    }
  }

  // Resilient offline fallback (works out-of-the-box on Vercel without DB configuration)
  return NextResponse.json({
    success: true,
    todayUtc,
    count: globalFallbackSlots.length,
    slots: globalFallbackSlots,
    mode: 'resilient_storage',
  });
}
  ...
  try {
    ...
    if (hasDbUrl) {
      try {
        ...
        // Also register in memory so it's always accessible
        globalFallbackSlots.unshift(createdSlot);
      } catch {
        // Database operation failed, gracefully fall back to resilient in-memory store
        createdSlot = createInMemorySlot(modelType as ModelId, providerWallet, apiKey, dailyQuota, remainingCredit, pricing);
      }
    } else {
      // Resilient mode without DATABASE_URL
      createdSlot = createInMemorySlot(modelType as ModelId, providerWallet, apiKey, dailyQuota, remainingCredit, pricing);
    }
